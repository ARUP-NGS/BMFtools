...
// If not set, zlib compresses all our files enormously.
    ...
    while(LIKELY((l = kseq_read(seq)) >= 0)) {
        if(UNLIKELY(++count % 1000000 == 0))
            fprintf(stderr, "[%s::%s] Number of records read: %llu.\n", __func__,
...
#if !NDEBUG
    fprintf(stderr, "[D:%s::%s] Total number of collapsed observations: %llu.\n", __func__, ifn_stream(infname), count);
#endif
    free(ks.s);
    gzclose(fp);
    gzclose(out_handle);
...
#if !NDEBUG
        if(UNLIKELY(++count % 1000000 == 0))
            fprintf(stderr, "[%s::%s] Number of records processed: %llu.\n", __func__,
