...
def main():
    ...
    if(args.BAM!=None):
        AMfile=args.BAM
        logger.write('\nSkipping alignment, before a BAM or SAM file was already provided.\n')
    else:
        #Sort the Fastq's
    ...
    if(args.sort==True):
        ...
        sortAMfile=AMfile
        print('Sorting was opted out of.')
        logger.write('Sorting was opted out of.')
    #Parse in *amfile
    ...
    SamReads1.close()
    SamReads2.close()
    #Note: problem is not coming from how the sam file is accessed. until_eof and regular are indistinguishable 
    
    #Parse in fastq files
    fr1 = SeqIO.parse(basename+'_reads1.sam','fastq')
    fr2 = SeqIO.parse(basename+'_reads2.sam','fastq')
    #Loop through Reads1 file
    ...
    #Fix pair 1's 
    SamReads1=pysam.Samfile(basename+'_reads1.sam','r')
    ...
    tempname = open(basename+'tempnames.txt','w+',0)
    SamReads2=pysam.Samfile(basename+'_reads2.sam','r')
    ...
    sam1 = basename+'-PE1-tf.sam'.format(trim,aligner)
    sam2 = basename+'-PE2-tf.sam'.format(trim,aligner)
    bam1 = basename+'.{}.{}.final-1.bam'.format(trim,aligner)
    bam2 = basename+'.{}.{}.final-2.bam'.format(trim,aligner)
    ...
    final_str='samtools merge -nr -h {} {} {} {}'.format(sam,final_bam,bam1,bam2)
...
def sam_sort(insam,outsam):
    ...
    import subprocess
    command_str0=str('grep "@SQ\|@PG\|@HD" {}'.format(insam))
    print(command_str0)
    subprocess.call(command_str0,stdout=output,shell=True)
    command_str=str('grep -v "@SQ\|@PG\|VN:\|@HD" {}'.format(insam))
    print(command_str)
    subprocess.call(command_str,stdout=tmp,shell=True)
    tmp.close()
    tmp=open(tmpname,'r')
    command_str1=str('sort -k1,1 {}'.format(tmpname))
    print(command_str1)
