...
static inline void dmp_process_write_full_pvalues(KingFisher_t *kfp, FILE *handle, int blen, tmpbuffers_t *tmp)
{
    ...
    for(int i = 0; i < kfp->readlen; ++i) {
        ...
        for(int j = 0; j < 4; ++j) {
            ...
            tmp_phreds[j] = igamc_pvalues(kfp->length, LOG10_TO_CHI2(kfp->phred_sums[i][j]));
        }
        if(tmp_max < 0) {
            tmp_max = 0;
        }
        ...
        tmp->cons_quals[i] = pvalue_to_phred(tmp_phreds[tmp_max]);
...
static inline void dmp_process_write_sub_chi2(KingFisher_t *kfp, FILE *handle, int blen, tmpbuffers_t *tmp)
{
    ...
    for(int i = 0; i < kfp->readlen; ++i) {
        ...
        for(int j = 0; j < 4; ++j) {
            ...
            tmp_phreds[j] = igamc_pvalues(kfp->nuc_counts[i][j], LOG10_TO_CHI2(kfp->phred_sums[i][j]));
        }
        if(tmp_max < 0) {
            tmp_max = 0;
        }
 ...
 * TODO: Use tmpvals_t object to avoid allocating and deallocating each of these.
 */
static inline void dmp_process_write_simple_chi2_sub(KingFisher_t *kfp, FILE *handle, int blen, tmpbuffers_t *tmp)
{
    int argmaxret;
    int tmpsum = 0;
    tmp->cons_seq_buffer[kfp->readlen] = '\0'; // Null-terminal cons_seq.
    for(int i = 0; i < kfp->readlen; ++i) {
        ...
        for(int j = 0; j < 4; ++j) {
            if(j != argmaxret) {
                tmpsum -= kfp->phred_sums[i][j];
            }
            else {
                tmpsum += kfp->phred_sums[i][j];
            }
        }
        tmp->cons_quals[i] = pvalue_to_phred(igamc_pvalues(kfp->length, LOG10_TO_CHI2(tmpsum)));
        if(tmp->cons_quals[i] < -1073741824) { // Underflow!
            tmp->cons_quals[i] = 3114;
        }
        // Final quality must be 2 or greater and at least one read in the family should support that base call.
