...
static void print_stats(famstats_t *stats, FILE *fp, famstats_fm_settings_t *settings)
{
    ...
    if(settings->skip_fp_fail)
        fprintf(fp, "#Number failing FP filters: %" PRIu64 "\n", stats->n_fp_fail);
    else
        fprintf(fp, "#Count for FP failed reads, still included in total counts: %" PRIu64 "\n", stats->n_fp_fail);
    fprintf(fp, "#Number failing FM filters: %" PRIu64 "\n", stats->n_fm_fail);
    fprintf(fp, "#Number failing MQ filters: %" PRIu64 "\n", stats->n_mq_fail);
    fprintf(fp, "#Number failing flag filters (secondary, supplementary): %" PRIu64 "\n", stats->n_flag_fail);
