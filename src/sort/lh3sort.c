...
void print_keyfield_info(struct keyfield *kf) {
	fprintf(stderr, "sword: %i. schar: %i. eword: %i.char: %i."
					"skipsblanks: %i. skipeblanks: %i. translate: %s.\n",
			kf->sword, kf->schar, kf->eword, kf->echar, kf->skipsblanks,
			kf->skipeblanks, kf->translate);
	if(kf->next != NULL) {
		print_keyfield_info(kf->next);
	}
}
