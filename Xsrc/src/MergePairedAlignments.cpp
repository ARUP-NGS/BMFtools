...
std::vector<std::string> splitString(std::string inStr, std::string delimiter) {
    // I can't see why this is segfaulting in some cases.
    // Don't use it, please! Keeping around so I can get it diagnosed later.
...
int main(int argc, char* argv[])
{
    ...
    inputFq2 = argv[3];
    if(argc == 4) {
        outputBam = splitByChar(inputBam, '.')[0] + ".tagged.bam";
    }
    else {
        outputBam = argv[4];
    }
    ...
    while (reader.GetNextAlignment(rec)) {
        ...
        for(int i = 0; i < tagsLen; i++) {
            ...
            if(TagPair[0].compare("BS") == 0) {
                BarcodeString = seq1.seq.substr(0, head) + TagPair[1] + seq2.seq.substr(0, head);
            }
        }
