...
#Align to ensembl
$bt2 $ens_ref -U $fq.unmapped.mir.ENSnc.fastq --un $1.unmapped.mir.ENSnc.ens.fastq -p $threads -S $1.mapped.ens.sam &> $1.ens.log
...
echo 'total reads unmapped to miRNA/ENSnc/ensembl small RNA databases is '$(wc -l $fq.unmapped.mir.ENSnc.ens.fastq) >> $1.report

#Align to RefSeq
$bt2 $refseq_ref -U $fq.unmapped.mir.ENSnc.ens.fastq --un $1.unmapped.mir.ENSnc.ens.rs.fastq -p $threads -S $1.mapped.rs.sam &> $1.rs.log
...
un=$(wc -l < $fq.unmapped.mir.ENSnc.ens.rs.fastq)
und=$(expr $un / 4)
